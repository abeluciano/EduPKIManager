    ...
    \begin{figure}[H]
        ...
        \fbox{
            \begin{minipage}[c][4.6cm][c]{0.88\textwidth}
                \centering
                \textbf{#1}\\[0.35cm]
                \small Inserte aqui la captura correspondiente del sistema.
            \end{minipage}
        }
        \caption{#2}
    \end{figure}
...
\begin{tikzpicture}[
    ...
    comp/.style={draw=EduBorder, rounded corners, fill=EduLight, minimum width=5.7cm, minimum height=0.9cm, align=center, font=\small},
    store/.style={draw=EduBorder, rounded corners, fill=EduGray, minimum width=5.7cm, minimum height=0.9cm, align=center, font=\small},
    arrow/.style={-Latex, thick, color=EduGreen}
]
    \node[comp] (front) {Frontend React + TypeScript\\Roles admin/user};
    \node[comp, below=of front] (api) {Django REST Framework\\Autenticacion, permisos y endpoints};
    \node[comp, below left=1.1cm and 1.2cm of api] (crypto) {Servicios PKI\\CA, certificados, CRL, OCSP, PDF, TLS};
    \node[store, below right=1.1cm and 1.2cm of api] (data) {Persistencia\\PostgreSQL, volumen CA, CRL, auditoria};
    \node[comp, below=1.2cm of crypto] (nginx) {Nginx HTTPS\\TLS 1.3 para edupkimanager.com};
    \node[comp, below=1.2cm of data] (scheduler) {Servicios auxiliares\\crl\_scheduler y tls\_init};

    \draw[arrow] (front) -- (api);
    \draw[arrow] (api) -- (crypto);
    \draw[arrow] (api) -- (data);
    \draw[arrow] (crypto) -- (data);
    \draw[arrow] (nginx) -- (front);
    \draw[arrow] (scheduler) -- (data);
    \draw[arrow] (scheduler) -- (crypto);
\end{tikzpicture}
\caption{Diagrama de Componentes de EduPKIManager}
\end{figure}
...
\begin{tikzpicture}[
    ...
    module/.style={draw=EduBorder, rounded corners, fill=EduLight, minimum width=4.3cm, minimum height=0.8cm, align=center, font=\small},
    arrow/.style={-Latex, thick, color=EduGreen}
]
    \node[module] (api) {api\\Vistas REST y permisos};
    \node[module, below left=of api] (ca) {ca\\Root, Intermediate, X.509};
    \node[module, below=of api] (validation) {validation\\Cadena, usos, CRL, OCSP};
    \node[module, below right=of api] (audit) {audit\\Hash encadenado};
    \node[module, below=of ca] (crl) {crl\\Publicacion PEM/DER};
    \node[module, below=of validation] (ocsp) {ocsp\\JSON y RFC 6960};
    \node[module, below=of audit] (pdf) {pdf\_sign\\Detached y PAdES};
    \node[module, below=of ocsp] (tls) {tls\\Bundle servidor y TLS 1.3};

    \draw[arrow] (api) -- (ca);
    \draw[arrow] (api) -- (validation);
    \draw[arrow] (api) -- (audit);
    \draw[arrow] (ca) -- (crl);
    \draw[arrow] (validation) -- (ocsp);
    \draw[arrow] (validation) -- (pdf);
    \draw[arrow] (ca) -- (tls);
    \draw[arrow] (pdf) -- (audit);
    \draw[arrow] (tls) -- (audit);
\end{tikzpicture}
\caption{Diagrama de Modulos del Backend}
\end{figure}
...
\begin{tikzpicture}[
    node distance=0.75cm,
    step/.style={draw=EduBorder, rounded corners, fill=EduLight, minimum width=8.8cm, minimum height=0.8cm, align=center, font=\small},
    decision/.style={draw=EduBorder, diamond, aspect=2.2, fill=EduGray, align=center, font=\small},
    arrow/.style={-Latex, thick, color=EduGreen}
]
    \node[step] (login) {Usuario autenticado envia solicitud};
    \node[decision, below=of login] (role) {Rol valido?};
    \node[step, below=of role] (ca) {Generar clave y certificado X.509 v3};
    \node[step, below=of ca] (sign) {Firmar con Intermediate CA};
    \node[step, below=of sign] (save) {Guardar registro en PostgreSQL};
    \node[step, below=of save] (audit) {Registrar evento en auditoria};
    \node[step, below=of audit] (response) {Responder certificado y clave privada};

    \draw[arrow] (login) -- (role);
    \draw[arrow] (role) -- node[right]{Si} (ca);
    \draw[arrow] (ca) -- (sign);
    \draw[arrow] (sign) -- (save);
    \draw[arrow] (save) -- (audit);
    \draw[arrow] (audit) -- (response);
\end{tikzpicture}
\caption{Diagrama de Flujo de Emision de Certificado}
\end{figure}
...
\begin{tikzpicture}[
    node distance=0.7cm,
    step/.style={draw=EduBorder, rounded corners, fill=EduLight, minimum width=9.2cm, minimum height=0.8cm, align=center, font=\small},
    arrow/.style={-Latex, thick, color=EduGreen}
]
    \node[step] (input) {Cargar PDF y firma JSON o PDF PAdES};
    \node[step, below=of input] (hash) {Calcular SHA-256 del documento};
    \node[step, below=of hash] (sig) {Verificar firma con clave publica del certificado};
    \node[step, below=of sig] (chain) {Validar cadena Root CA -> Intermediate CA -> Certificado};
    \node[step, below=of chain] (rev) {Consultar estado local, CRL y OCSP};
    \node[step, below=of rev] (audit) {Registrar operacion y resultado};
    \node[step, below=of audit] (result) {Mostrar reporte de trazabilidad};

    \draw[arrow] (input) -- (hash);
    \draw[arrow] (hash) -- (sig);
    \draw[arrow] (sig) -- (chain);
    \draw[arrow] (chain) -- (rev);
    \draw[arrow] (rev) -- (audit);
    \draw[arrow] (audit) -- (result);
\end{tikzpicture}
\caption{Diagrama de Flujo de Verificacion de Firma PDF}
\end{figure}
...
\centering
\begin{tikzpicture}[
    lifeline/.style={draw=EduBorder, fill=EduLight, minimum width=2.8cm, minimum height=0.65cm, align=center, font=\small},
    msg/.style={-Latex, thick, color=EduGreen},
    every node/.style={font=\small}
]
    \node[lifeline] (client) at (0,0) {Cliente};
    \node[lifeline] (nginx) at (4,0) {Nginx};
    \node[lifeline] (ca) at (8,0) {CA propia};

    \draw[dashed] (client) -- (0,-5.4);
    \draw[dashed] (nginx) -- (4,-5.4);
    \draw[dashed] (ca) -- (8,-5.4);

    \coordinate (ca1) at (8,-0.8);
    \coordinate (ng1) at (4,-0.8);
    \coordinate (cl2) at (0,-1.6);
    \coordinate (ng2) at (4,-1.6);
    \coordinate (ng3) at (4,-2.4);
    \coordinate (cl3) at (0,-2.4);
    \coordinate (cl4) at (0,-3.2);
    \coordinate (ca4) at (8,-3.2);
    \coordinate (cl5) at (0,-4.0);
    \coordinate (ng5) at (4,-4.0);
    \coordinate (ng6) at (4,-4.8);
    \coordinate (cl6) at (0,-4.8);

    \draw[msg] (ca1) -- node[above]{emite certificado servidor} (ng1);
    \draw[msg] (cl2) -- node[above]{ClientHello TLS 1.3} (ng2);
    \draw[msg] (ng3) -- node[above]{ServerHello + certificado} (cl3);
    \draw[msg] (cl4) -- node[above]{valida Root CA confiable} (ca4);
    \draw[msg] (cl5) -- node[above]{claves de sesion} (ng5);
    \draw[msg] (ng6) -- node[above]{canal HTTPS TLS 1.3} (cl6);
\end{tikzpicture}
\caption{Diagrama de Secuencia de Handshake TLS 1.3}
\end{figure}
...
EduPKIManager/
  ...
  frontend/
    src/
      ...
      tools/      # Firma, OCSP, CRL, TLS y auditoria
  scripts/
...
\captura{Login de EduPKIManager}{Interfaz de inicio de sesion}
...
\captura{Panel administrador}{Interfaz de gestion completa de certificados}

\subsection{Portal Usuario Final}

El portal usuario final reduce el alcance a certificados propios. Desde esta vista se puede solicitar un certificado personal, ver su estado, descargarlo y utilizarlo para operaciones de firma y validacion.

\captura{Portal de usuario}{Interfaz del usuario final}

\subsection{Operaciones PKI}

La vista de operaciones agrupa herramientas de firma PDF, verificacion PDF, confianza X.509, OCSP, CA/CRL, TLS 1.3 y auditoria.

\captura{Operaciones PKI}{Interfaz de herramientas criptograficas}
