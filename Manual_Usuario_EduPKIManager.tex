...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Usuario final:} consulta certificados emitidos a su propietario por el administrador, firma documentos y verifica firmas PDF.
\end{itemize}
...
Universidad la Salle & \texttt{lasalle / lasalle123} & Consulta de certificados asignados, descarga, firma PDF y verificacion PDF. \\
Abel Aragon & \texttt{abel.aragon / abel123} & Consulta de certificados asignados, descarga, firma PDF y verificacion PDF. \\
Carlos Mijail & \texttt{carlos.mijail / carlos123} & Consulta de certificados asignados, descarga, firma PDF y verificacion PDF. \\
Josshua Flores & \texttt{josshua.flores / josshua123} & Consulta de certificados asignados, descarga, firma PDF y verificacion PDF. \\
Marco Alatrista & \texttt{marco.alatrista / marco123} & Consulta de certificados asignados, descarga, firma PDF y verificacion PDF. \\
\bottomrule
\end{longtable}
...
\begin{itemize}[leftmargin=*]
    ...
    \item Si el rol es \textbf{user}, aparecen las vistas \textit{Mi portal} y \textit{Firmas PDF}.
...
El portal de usuario final esta orientado a consulta y uso de certificados asignados. El usuario no emite certificados; solo puede ver, descargar y usar los certificados que el administrador emitio para su propietario.

\captura{portal_usuario_final}{Portal de usuario final}{Vista Mi portal para usuario final}

\subsection{Consultar Certificados Asignados}

Para revisar certificados asignados:

\begin{enumerate}[leftmargin=*]
    \item Inicie sesion con alguno de los cinco usuarios finales definidos.
    \item Ingrese a \textit{Mi portal}.
    \item Revise la lista de certificados emitidos por el administrador para ese propietario.
    \item Descargue el certificado si necesita usarlo como evidencia o validarlo externamente.
    \item Use el boton \textit{Actualizar} para refrescar el listado.
\end{enumerate}

\nota{El usuario final no puede emitir, renovar, suspender ni revocar certificados. Tampoco ve en su frontend los modulos de confianza X.509, OCSP, CA/CRL, TLS o auditoria.}
...
\begin{enumerate}[leftmargin=*]
    \item Ingrese a \textit{Operaciones PKI} como administrador o \textit{Firmas PDF} como usuario final.
...
\begin{enumerate}[leftmargin=*]
    ...
    \item Emitir un certificado de usuario para uno de los cinco propietarios definidos.
